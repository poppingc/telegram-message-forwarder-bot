...
def work(client, message):
    caption = None
    LOG.info('new mesg word')
    msg = message.text.html[1:]
    if remove_strings:
      ...
      try:
        for chat in chats_data[message.chat.id]:
          if caption:
            ...
            LOG.info('new mesg word')
            app.send_message(chat, msg[1:], parse_mode="html")
          else:
            message.copy(chat)
...
@app.on_message(filters.user(sudo_users) & filters.command(["fwd", "forward", "docs"]), group=1)
def forward(app, message):
    LOG.info(message.command)
    if message.command.__contains__('docs'):
      LOG.info('contains__')
      chat = get_formatted_chat(message.chat.id, app)
      app.send_message(chat, 'docs.cheersland.org')
    else:
